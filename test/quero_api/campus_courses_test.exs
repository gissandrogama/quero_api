defmodule QueroApi.CampusCoursesTest do
  ...
  describe "campus_courses" do
    ...
    test "get_campus_course!/1 returns the campus_course with given id", %{
      campu_course: campu_course
    } do
    ...
    test "create_campus_course/1 with valid data creates a campus_course", %{
      ...
      assert {:ok, %CampusCourse{} = campus_course} =
               CampusCourses.create_campus_course(%{campu_id: campu.id, course_id: course.id})
    ...
    test "update_campus_course/2 with valid data updates the campus_course", %{
      campu_course: campu_course
    } do
      campu = campus_fixture()
      course = courses_fixture()

      assert {:ok, %CampusCourse{}} =
               CampusCourses.update_campus_course(campu_course, %{
                 campu_id: campu.id,
                 course_id: course.id
               })
    end

    test "update_campus_course/2 with invalid data returns error changeset", %{
      campu_course: campu_course
    } do
      assert {:error, %Ecto.Changeset{}} =
               CampusCourses.update_campus_course(campu_course, @invalid_attrs)
    ...
    test "delete_campus_course/1 deletes the campus_course" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        CampusCourses.get_campus_course!(campus_course.id)
      end
    end
